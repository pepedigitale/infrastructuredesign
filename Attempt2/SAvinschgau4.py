...
def find_overtake_km(dep_slow, dep_fast, direction):
    """
    Calcola il km in cui il treno veloce raggiunge (sorpassa) quello lento
    nella stessa direzione, usando traiettorie lineari.
    Restituisce il km di sorpasso o None se non avviene nel tratto comune.

    Entrambi i treni percorrono L km nella stessa direzione:
      ME→MA: km(t) = (t - dep) / travel * L        (cresce da 0 a L)
      MA→ME: km(t) = (1 - (t - dep) / travel) * L  (decresce da L a 0)

    Posizioni uguali (caso ME→MA, analogo per MA→ME):
      (t - dep_slow) / SLOW  =  (t - dep_fast) / FAST
      FAST*(t - dep_slow)    =  SLOW*(t - dep_fast)
      t*(FAST - SLOW)        =  FAST*dep_slow - SLOW*dep_fast
      t_meet = (FAST*dep_slow - SLOW*dep_fast) / (FAST - SLOW)

    Il sorpasso avviene solo se:
      - t_meet è nell'intervallo in cui entrambi sono in viaggio
      - il veloce era dietro al lento all'inizio (dep_fast > dep_slow, cioè
        il veloce parte dopo; se parte prima non c'è sorpasso nel senso operativo)
    """
    # Il sorpasso è fisicamente possibile solo se il veloce parte DOPO il lento
    # ma entro GAP_MIN minuti (altrimenti il lento è già arrivato)
    if dep_fast <= dep_slow:
        return None
    if dep_fast - dep_slow >= GAP_MIN_MINUTES:
        return None

    denom = FAST_TRAVEL_MIN - SLOW_TRAVEL_MIN   # 58 - 84 = -26  (sempre < 0)
    # Nota: denom è negativo, il che è corretto — inverte il segno di entrambi
    # numeratore e denominatore in modo consistente.

    if direction == "ME→MA":
        numer = FAST_TRAVEL_MIN * dep_slow - SLOW_TRAVEL_MIN * dep_fast
        t_meet = numer / denom

        arr_slow = dep_slow + SLOW_TRAVEL_MIN
        arr_fast = dep_fast + FAST_TRAVEL_MIN
        t_start  = dep_fast          # il veloce deve essere già partito
        t_end    = min(arr_slow, arr_fast)

        if t_meet < t_start or t_meet > t_end:
            return None

        km_meet = (t_meet - dep_slow) / SLOW_TRAVEL_MIN * LINE_LENGTH_KM

    else:  # MA→ME: posizione = (1 - (t-dep)/travel)*L, uguale quando stessa formula
        numer = FAST_TRAVEL_MIN * dep_slow - SLOW_TRAVEL_MIN * dep_fast
        t_meet = numer / denom
        ...
        # Per MA→ME il km decresce: km = (1 - frac)*L
        km_meet = (1.0 - (t_meet - dep_slow) / SLOW_TRAVEL_MIN) * LINE_LENGTH_KM

    km_meet = max(0.0, min(LINE_LENGTH_KM, km_meet))
    return km_meet
...
def compute_cost(solution, n_copies=5):
    ...
    2. Per ogni coppia (treno ME→MA, treno MA→ME), trova il km di incrocio
       (treni in senso opposto).
    3. Per ogni coppia di treni nella stessa direzione in cui uno è lento e
       l'altro è veloce, trova il km di sorpasso (overtaking).
    4. Per ogni incontro (incrocio o sorpasso), individua la sezione di blocco.
    5. Se entrambi gli estremi della sezione hanno 2+ binari → costo 0.
       Altrimenti la sezione va raddoppiata.
    6. Somma le lunghezze delle sezioni da raddoppiare (senza contare due volte).
    
    Restituisce: (costo_totale_km, set di indici sezioni da raddoppiare, lista incontri)
    Ogni incontro ha un campo "kind": "crossing" o "overtaking".
    """
    ...
    # ------------------------------------------------------------------
    # 1. Incroci: treni in direzione opposta
    # ------------------------------------------------------------------
    for a in me_ma_trains:
        for b in ma_me_trains:
            ...
            if sec["tracks_start"] >= 2 and sec["tracks_end"] >= 2:
                pass  # incrocio gestito gratuitamente in stazione a doppio binario
            else:
                sections_to_double.add(sec_idx)

    # ------------------------------------------------------------------
    # 2. Sorpassi: treni nella stessa direzione (veloce raggiunge lento)
    # ------------------------------------------------------------------
    for direction_group in (me_ma_trains, ma_me_trains):
        slow_trains = [t for t in direction_group if t["train_def"]["type"] == "slow"]
        fast_trains = [t for t in direction_group if t["train_def"]["type"] == "fast"]
        direction   = direction_group[0]["train_def"]["dir"] if direction_group else None

        for slow_t in slow_trains:
            for fast_t in fast_trains:
                km_over = find_overtake_km(
                    slow_t["departure_abs_min"],
                    fast_t["departure_abs_min"],
                    direction,
                )
                if km_over is None:
                    continue

                sec_idx = find_section_at_km(km_over)
                sec = BLOCK_SECTIONS[sec_idx]
                ...
                if sec["tracks_start"] >= 2 and sec["tracks_end"] >= 2:
                    pass  # sorpasso gestito gratuitamente in stazione a doppio binario
                else:
                    sections_to_double.add(sec_idx)
